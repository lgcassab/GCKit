...
- (void)dealloc
{
	...
	for (GCHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[GCHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[GCHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
{
	if ([operation isKindOfClass:[GCHTTPRequest class]]) {
		
		GCHTTPRequest *request = (GCHTTPRequest *)operation;
...
// Only add GCHTTPRequests to this queue!!
- (void)addOperation:(NSOperation *)operation
{
	if (![operation isKindOfClass:[GCHTTPRequest class]]) {
		[NSException raise:@"AttemptToAddInvalidRequest" format:@"Attempted to add an object that was not an GCHTTPRequest to an GCNetworkQueue"];
	}
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				GCHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(GCHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	...
	if ([self downloadProgressDelegate]) {
		[GCHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(GCHTTPRequest *)request didSendBytes:(long long)bytes
{
	...
	if ([self uploadProgressDelegate]) {
		[GCHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
...
{
	GCNetworkQueue *newQueue = [[[self class] alloc] init];
