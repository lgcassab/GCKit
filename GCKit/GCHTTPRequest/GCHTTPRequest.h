...
@class GCDataDecompressor;

extern NSString *GCHTTPRequestVersion;
...
typedef enum _GCAuthenticationState {
	GCNoAuthenticationNeededYet = 0,
	GCHTTPAuthenticationNeeded = 1,
	GCProxyAuthenticationNeeded = 2
} GCAuthenticationState;

typedef enum _GCNetworkErrorType {
    GCConnectionFailureErrorType = 1,
    GCRequestTimedOutErrorType = 2,
    GCAuthenticationErrorType = 3,
    GCRequestCancelledErrorType = 4,
    GCUnableToCreateRequestErrorType = 5,
    GCInternalErrorWhileBuildingRequestType  = 6,
    GCInternalErrorWhileApplyingCredentialsType  = 7,
	GCFileManagementError = 8,
	GCTooMuchRedirectionErrorType = 9,
	GCUnhandledExceptionError = 10,
	GCCompressionError = 11
	
} GCNetworkErrorType;


// The error domain that all errors generated by GCHTTPRequest use
...
extern unsigned long const GCWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^GCBasicBlock)(void);
typedef void (^GCHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^GCSizeBlock)(long long size);
typedef void (^GCProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^GCDataBlock)(NSData *data);
#endif

@interface GCHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the GCHTTPRequestDelegate protocol
	id <GCHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but GCNetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <GCHTTPRequestDelegate, GCProgressDelegate> queue;
	...
	// Automatically set to true in GCFormDataRequests when using setFile:forKey:
	...
	// Set to true when GCHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = GCConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <GCProgressDelegate> uploadProgressDelegate;
	...
	id <GCProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force GCHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, GCHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, GCHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	// The dialog will not be shown if your delegate responds to proxyAuthenticationNeededForRequest:
	// Default is YES (bGCcally, because most people won't want the hassle of adding support for authenticating proxies to their apps)
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - GCHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	// Tells GCHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let GCHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// GCHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See GCAuthenticationState values above. 0 == default == No authentication needed yet
	GCAuthenticationState authenticationNeeded;
	
	// When YES, GCHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops GCHTTPRequest cleaning up a temporary post body)
	...
	// Will be GCHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [GCHTTPRequest setDefaultCache:cache] to configure a default cache
	id <GCCacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See GCCacheDelegate.h for possible values
	GCCachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See GCCacheDelegate.h for possible values
	GCCacheStoragePolicy cacheStoragePolicy;
	...
	GCDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using GCHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
...
// Construct a bGCc authentication header from the username and password supplied, and add it to the request headers
...
// Used internally to record bandwidth use, and by GCInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, GCHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
// Used for generating Authorization header when using bGCc authentication when shouldPresentCredentialsBeforeChallenge is true
// And also by GCS3Request
...
@property (assign) GCHTTPRequest *mainRequest;
...
@property (assign, readonly) GCAuthenticationState authenticationNeeded;
...
@property (assign) id <GCCacheDelegate> downloadCache;
@property (assign) GCCachePolicy cachePolicy;
@property (assign) GCCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) GCDataDecompressor *dataDecompressor;
