...
@interface GCDataDecompressor ()
+ (NSError *)inflateErrorWithCode:(int)code;
@end;

@implementation GCDataDecompressor

+ (id)decompressor
{
	GCDataDecompressor *decompressor = [[[self alloc] init] autorelease];
...
+ (NSData *)uncompressData:(NSData*)compressedData error:(NSError **)err
{
	...
	NSData *outputData = [[GCDataDecompressor decompressor] uncompressBytes:(Bytef *)[compressedData bytes] length:[compressedData length] error:&theError];
	if (theError) {
		if (err) {
			*err = theError;
		}
...
+ (BOOL)uncompressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath error:(NSError **)err
{
	...
	if (![fileManager createFileAtPath:destinationPath contents:[NSData data] attributes:nil]) {
		if (err) {
			*err = [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCCompressionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Decompression of %@ failed because we were to create a file at %@",sourcePath,destinationPath],NSLocalizedDescriptionKey,nil]];
		}
		return NO;
	}
	...
	if (![fileManager fileExistsAtPath:sourcePath]) {
		if (err) {
			*err = [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCCompressionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Decompression of %@ failed the file does not exist",sourcePath],NSLocalizedDescriptionKey,nil]];
		}
		return NO;
	}
...
+ (NSError *)inflateErrorWithCode:(int)code
{
	return [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCCompressionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Decompression of data failed with code %d",code],NSLocalizedDescriptionKey,nil]];
}
