...
static GCAuthenticationDialog *sharedDialog = nil;
BOOL isDismissingGC = NO;
...
@implementation GCAutorotatingViewController
...
@interface GCAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation GCAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [GCAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(GCHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == GCProxyAuthenticationNeeded) {
			[sharedDialog setType:GCProxyAuthenticationType];
		} else {
			[sharedDialog setType:GCStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[GCAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == GCProxyAuthenticationType) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (GCHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (GCHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		GCHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (GCHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == GCStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
