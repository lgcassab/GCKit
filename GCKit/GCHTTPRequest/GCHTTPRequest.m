...
//  http://allseeing-i.com/GCHTTPRequest
//
...
#import "GCAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "GCInputStream.h"
#import "GCDataDecompressor.h"
#import "GCDataCompressor.h"

// Automatically set on build
NSString *GCHTTPRequestVersion = @"v1.8.1-28 2011-08-07";

static NSString *defaultUserAgent = nil;

NSString* const GCNetworkRequestErrorDomain = @"GCHTTPRequestErrorDomain";

static NSString *GCHTTPRequestRunLoopMode = @"GCHTTPRequestRunLoopMode";
...
const int GCRedirectionLimit = 5;
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((GCHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *GCRequestCancelledError;
static NSError *GCRequestTimedOutError;
static NSError *GCAuthenticationError;
static NSError *GCUnableToCreateRequestError;
static NSError *GCTooMuchRedirectionError;
...
unsigned long const GCWWANBandwidthThrottleAmount = 14800;
...
// When using GCAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <GCCacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [GCHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
+ (void)performInvocation:(NSInvocation *)invocation onTarget:(id *)target releGCngObject:(id)objectToRelease;
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(GCHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(GCBasicBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(GCBasicBlock)block;
#endif
...
@property (assign) GCAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) GCHTTPRequest *PACFileRequest;
...
@implementation GCHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [GCHTTPRequest class]) {
		...
		GCRequestTimedOutError = [[NSError alloc] initWithDomain:GCNetworkRequestErrorDomain code:GCRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		GCAuthenticationError = [[NSError alloc] initWithDomain:GCNetworkRequestErrorDomain code:GCAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		GCRequestCancelledError = [[NSError alloc] initWithDomain:GCNetworkRequestErrorDomain code:GCRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		GCUnableToCreateRequestError = [[NSError alloc] initWithDomain:GCNetworkRequestErrorDomain code:GCUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		GCTooMuchRedirectionError = [[NSError alloc] initWithDomain:GCNetworkRequestErrorDomain code:GCTooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[GCHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <GCCacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:GCUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <GCCacheDelegate>)cache andCachePolicy:(GCCachePolicy)policy
{
	GCHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:GCNoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![GCDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [GCDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	[self failWithError:GCRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [GCDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:GCHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([GCHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an GCNetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:GCUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an GCNetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (GCS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (GCAskServerIfModifiedWhenStaleCachePolicy|GCAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		// If we have bGCc authentication explicitly set and a username and password set on the request, add a bGCc auth header
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			NSLog(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BGCC. Will send credentials without waiting for an authentication challenge",self);	
			#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					// If the Authentication key is not set, these credentials were stored after a username and password set on a previous request passed bGCc authentication
					// When this happens, we'll need to create the Authorization header ourselves
						...
						#if DEBUG_HTTP_AUTHENTICATION
						NSLog(@"[AUTH] Request %@ found cached BGCC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
						#endif
					}
				}
			}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [GCHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[GCInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[GCInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[GCInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[GCInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[GCHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("GCStreamID"), [[self connectionInfo] objectForKey:@"id"]);
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[GCHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[GCHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:GCRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[GCHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
// Used by GCNetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (GCHTTPRequest *)HEADRequest
{
	GCHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[GCHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[GCHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[GCHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[GCHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[GCHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[GCHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[GCHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[GCHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[GCHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[GCHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[GCHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[GCHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[GCHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[GCHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[GCHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        SEL callback = @selector(performInvocation:onTarget:releGCngObject:);
        NSMethodSignature *cbSignature = [GCHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[GCHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	NSLog(@"[STATUS] Request %@: %@",self,(theError == GCRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != GCAuthenticationErrorType && [theError code] != GCTooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
	...
	// If we have cached data, use it and ignore the error when using GCFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & GCFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	GCHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an GCNetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:GCHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:GCProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
	...
	if (![self authenticationNeeded]) {
		...
		// Did we get here without an authentication challenge? (which can happen when shouldPresentCredentialsBeforeChallenge is YES and bGCc auth was successful)
		if (!requestAuthentication && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic] && [self username] && [self password] && [self useSessionPersistence]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			NSLog(@"[AUTH] Request %@ passed BGCC authentication, and will save credentials in the session store for future use",self);
			#endif
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[GCHTTPRequest addSessionCookie:cookie];
		}
	}
...
- (BOOL)willRedirect
{
	...
	// Note that GCHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[GCHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[GCHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	GCHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an GCNetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [GCHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [GCHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		NSLog(@"[AUTH] Request %@ received credentials from its delegate or an GCAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		NSLog(@"[AUTH] Request %@ had authentication cancelled by its delegate or an GCAuthenticationDialog",self);
	#endif
...
- (void)failAuthentication
{
	[self failWithError:GCAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[GCAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
	#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == GCProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (!CFHTTPAuthenticationIsValid(requestAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			if ([self showAuthenticationDialog]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				NSLog(@"[AUTH] Request %@ will ask GCAuthenticationDialog for credentials",self);
				#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[GCHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[GCDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[GCHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[GCDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
			...
			// Response should already have been inflated, move the temporary file to the destination path
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
	...
	// If request has asked delegate or GCAuthenticationDialog for credentials
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-GCHTTPRequest-Response-Status-Code"] intValue]];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:GCNetworkRequestErrorDomain code:GCInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (GCHTTPRequest does not support file:// urls)
	...
	// Create an GCHTTPRequest to fetch the PAC file
	GCHTTPRequest *PACRequest = [GCHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	GCHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[GCHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[GCHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[GCHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[GCHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[GCHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:GCCacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and GCHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since GCHTTPRequest only works on 10.5+
				OSErr err;
...
- (void)performThrottling
{
	...
	[GCHTTPRequest measureBandwidthUsage];
	if ([GCHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[GCHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an GCNetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[GCHTTPRequest throttleBandwidthForWWANUsingLimit:GCWWANBandwidthThrottleAmount];
	} else {
		[GCHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[GCHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[GCHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[GCHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[GCReachability reachabilityForInternetConnection] startNotifier];
...
{
	return ([[GCReachability reachabilityForInternetConnection] currentReachabilityStatus] == GCReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [GCHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <GCCacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <GCCacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(GCHTTPRequest *)request
{
...
+ (NSDate *)expiryDateForRequest:(GCHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [GCHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(GCBasicBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(GCHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(GCBasicBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(GCBasicBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(GCProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(GCProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(GCSizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(GCSizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(GCDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(GCBasicBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(GCBasicBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(GCBasicBlock)aRedirectBlock
{
