...
#import "GCDownloadCache.h"
#import "GCHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static GCDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface GCDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation GCDownloadCache

- (id)init
{
	self = [super init];
	[self setShouldRespectCacheControlHeaders:YES];
	[self setDefaultCachePolicy:GCUseDefaultCachePolicy];
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"GCHTTPRequestCache"]];
			}
		}
	}
	return sharedCache;
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:GCCacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (void)updateExpiryForRequest:(GCHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-GCHTTPRequest-Expires"];
...
{
  return [GCHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}
...
- (void)storeResponseForRequest:(GCHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & GCDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-GCHTTPRequest-Expires' header
	...
	// We store this as a timestamp to make reading it eGCer as NSDateFormatter is quite expensive

	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-GCHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-GCHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(GCHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == GCCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)isCachedDataCurrentForRequest:(GCHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-GCHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-GCHTTPRequest-Expires"];
		if (expires) {
...
{
	[[self accessLock] lock];
	GCCachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(GCCachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = GCAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(GCCacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == GCCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/GCHTTPRequest/ is cached the same as http://allseeing-i.com/GCHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(GCHTTPRequest *)request
{
	...
	if ([request cachePolicy] & GCOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & GCFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & GCAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & GCAskServerIfModifiedCachePolicy) {
