...
NSString *const kGCKeychainErrorDomain = @"com.gccore.GCKeychain";

NSString *const kGCKeychainAccountKey = @"acct";
NSString *const kGCKeychainCreatedAtKey = @"cdat";
NSString *const kGCKeychainClassKey = @"labl";
NSString *const kGCKeychainDescriptionKey = @"desc";
NSString *const kGCKeychainLabelKey = @"labl";
NSString *const kGCKeychainLastModifiedKey = @"mdat";
NSString *const kGCKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef GCKeychainAccessibilityType = NULL;
#endif

@interface GCKeychain ()
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = GCKeychainErrorBadArguments;
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (GCKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return GCKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (GCKeychainAccessibilityType) {
		CFRelease(GCKeychainAccessibilityType);
	}
	GCKeychainAccessibilityType = accessibilityType;
}
#endif
...
+ (NSError *)_errorWithCode:(OSStatus) code {
    ...
    switch (code) {
        ...
        case GCKeychainErrorBadArguments: message = @"Some of the arguments were invalid"; break;
    ...
    return [NSError errorWithDomain:kGCKeychainErrorDomain
                               code:code
                           userInfo:userInfo];
}
